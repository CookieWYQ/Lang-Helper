__all__ = ['JsonLangOperation']

import os
import json


class JsonLangOperation:
    """
    ...
    def __init__(self, lang_path: str) -> None:
        ...
        # 检查是不是json文件
        if not lang_path.endswith('.json'):
            raise ValueError(f'{lang_path} is not a json file')
    ...
    def contrast_lang(self, other: 'JsonLangOperation') -> dict[str, str]:
        """
        对比当前语言文件与另一个语言文件的差异。
    ...
    def write_lang_by_obj(self, other: 'JsonLangOperation', cover: bool = True) -> None:
        """
        将另一个语言文件的内容写入当前语言文件。
    ...
    def complete_lang_by_obj(self, other: 'JsonLangOperation') -> None:
        """
...
if __name__ == '__main__':
    ...
    en_lang_op = JsonLangOperation('fy_ok/en_us.lang')
    zh_lang_op = JsonLangOperation('fy_ok/zh_cn.lang')
    # 输出两种语言文件的差异
