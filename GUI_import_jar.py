...
from transfromAPIWithJinShan import translate_with_dict
from LangOperation import *
from JsonLangOperation import *
from tool import *
from extract_jar_with_progress import *
...
jar_path: str = "fy/test.jar"
main_lang_path: str
contrast_lang_path: str
...
i_: int = 0
all__: int = 1
finish_extract_jar: bool = False


def extract_jar_all(path: str) -> None:  # TODO: 优化
    import glob
    extract_jar_with_progress_window(path, f'fy_/{os.path.basename(path)}')
    ...
    main_label = tk.Label(show_file_win, text="主语言")
    main_label.place(x=10, y=5)
    main_chooser = ttk.Combobox(show_file_win, values=l1, width=50, state="readonly")
    main_chooser.current(0)
    main_chooser.place(x=10, y=30)

    contrast_label = tk.Label(show_file_win, text="对比语言")
    contrast_label.place(x=10, y=70)
    contrast_chooser = ttk.Combobox(show_file_win, values=l1, width=50, state="readonly")
    contrast_chooser.current(0)
    contrast_chooser.place(x=10, y=95)

    def finish():
        global main_lang, contrast_lang, finish_extract_jar
        if main_chooser.get() == contrast_chooser.get():
            messagebox.showerror("文件路径", "文件路径不能相同！")
            sys.exit()
        if json_or_lang_path(main_chooser.get()):
            main_lang = JsonLangOperation(os.path.abspath(main_chooser.get()))
        elif not json_or_lang_path(main_chooser.get()):
            main_lang = LangOperation(os.path.abspath(main_chooser.get()))
        else:
            messagebox.showerror("文件路径(main)", "文件错误！只允许json或lang文件！")
            sys.exit()
        if json_or_lang_path(contrast_chooser.get()):
            contrast_lang = JsonLangOperation(os.path.abspath(contrast_chooser.get()))
        elif not json_or_lang_path(contrast_chooser.get()):
            contrast_lang = LangOperation(os.path.abspath(contrast_chooser.get()))
        else:
            messagebox.showerror("文件路径(contrast)", "文件错误！只允许json或lang文件！")
            sys.exit()
        show_file_win.destroy()
        finish_extract_jar = True
        win.focus_force()
        win.destroy()
        return

    finish_button = tk.Button(show_file_win, text="完成", command=finish)
    finish_button.place(x=10, y=130)
    show_file_win.mainloop()
    return
...
def init():
    global config, win, \
        ...
        USE_RELOAD_TIP, file_path, file_type, keys_list, save_key, USE_SAVE_TIP, jar_path, \
        main_lang, contrast_lang, input_vars, inputs, lens_key, states, show_order, \
        interval, show_order, show_num, USE_SAVE_TIP_WHEN_SETTING_CLOSE, USE_SAVE_TIP_WHEN_MAIN_CLOSE, translate_api

    if not os.path.exists('config_jar/config.json'):
        messagebox.showerror("文件路径", "配置文件不存在！")
        sys.exit()

    with open('config_jar/config.json', 'r', encoding='utf-8') as f:
        config = json.load(f)

    if not os.path.exists(config["jar_path"]):
        messagebox.showerror("文件路径", "jar文件路径错误！")
        sys.exit()
    ...
    if is_jar(jar_path):
        if ALLOW_RELOAD_LANG:
            ...
            messagebox.showinfo("jar文件", "jar文件已解压！")
    else:
        messagebox.showerror("文件路径", "jar文件路径错误！")
        sys.exit()
...
def save():
    ...
    main_lang.save_lang()
    compress_folder_to_jar_with_progress_window(jar_path, jar_path.split(".")[0] + "_已汉化.jar")
...
def ask_lang_path() -> str:
    return filedialog.askopenfilename(title="选择语言文件", filetypes=[("语言文件", "*.lang")])
...
def show_setting_window():
    ...
    def save_to_hard_drive():
        ...
        config['jar_path'] = jar_path_entry.get()
        extract_jar_all(config["jar_path"])
    ...
    def ask_jar_path():
        ...
        if fp:
            jar_path = fp
            jar_path_entry.delete(0, tk.END)
            jar_path_entry.insert(0, fp)
            messagebox.showinfo("路径", "成功选择路径！")
        else:
            messagebox.showerror("路径", "路径为空！")
            return

    contrast_lang_path_label = tk.Label(setting_window, text="jar文件路径:")
    contrast_lang_path_label.place(x=10, y=60)
    jar_path_entry = tk.Entry(setting_window, width=30, font=("Arial", 10), bd=1, relief="solid", bg="white",
    ...
    jar_path_entry.place(x=10, y=80)
    jar_path_entry.insert(0, config["jar_path"])
    jar_path_button = tk.Button(setting_window, text="选择", command=ask_jar_path,
                                font=("Arial", 10),
    ...
    jar_path_button.place(x=240, y=80)
...
def translate__dict() -> None:
    text = simpledialog.askstring("输入", "请输入要翻译的内容")
    if text is not None:
        try:
            text_translated = translate_with_dict(text)
        except IndexError:
            text_translated = translate_with_dict(text)
        if text_translated is not None:
            messagebox.showinfo("提示", f"翻译结果为：\n{text_translated}")
        else:
...
right_click_menu.add_command(label="词典(金山词典, 英转汉)", command=translate__dict)
right_click_menu.add_separator()
